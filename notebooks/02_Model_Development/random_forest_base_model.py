# Random Forest Base Model - Colab Notebook Cell Structure
...
model = RandomForestClassifier(n_estimators=100, random_state=random_seed, n_jobs=n_jobs)
...
print('Random Forest Base Model Cross-Validation Results:')
...
plt.title('Random Forest Base Model Metrics')
plt.ylabel('Score')
plt.ylim(0, 1)
plt.tight_layout()
...
model.fit(X, y)
joblib.dump(model, f'{model_dir}/random_forest_base_model.pkl')
pd.DataFrame({'y_true': y, 'y_pred': y_pred, 'y_proba': y_proba}).to_csv(f'{results_dir}/cv_predictions.csv', index=False)

print('Random Forest Base Model training, evaluation, and saving complete.') 