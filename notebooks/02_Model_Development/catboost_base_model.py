...
try:
    clf = CatBoostClassifier(verbose=0, random_seed=random_seed, task_type='GPU', devices='0')
except Exception:
    clf = CatBoostClassifier(verbose=0, random_seed=random_seed)
...
clf.fit(X, y)
clf.save_model(os.path.join(model_dir, 'catboost_base_model.cbm'))
