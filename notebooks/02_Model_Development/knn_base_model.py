# K-Nearest Neighbors (KNN) Base Model - Colab Notebook Cell Structure
...
print('KNN Base Model Cross-Validation Results:')
...
plt.title('KNN Base Model Metrics')
plt.ylabel('Score')
plt.ylim(0, 1)
plt.tight_layout()
...
model.fit(X, y)
joblib.dump(model, f'{model_dir}/knn_base_model.pkl')
pd.DataFrame({'y_true': y, 'y_pred': y_pred, 'y_proba': y_proba}).to_csv(f'{results_dir}/cv_predictions.csv', index=False)

print('KNN Base Model training, evaluation, and saving complete.') 