...
model = RandomForestClassifier(**best_params, random_state=random_seed, n_jobs=n_jobs)
skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=random_seed)
...
print('Random Forest Tuned Model Cross-Validation Results:')
...
plt.title('Random Forest Tuned Model Metrics')
plt.ylabel('Score')
plt.ylim(0, 1)
plt.tight_layout()
...
model.fit(X, y)
joblib.dump(model, f'{model_dir}/random_forest_tuned_model.pkl')
...
print('Random Forest Tuned Model training, evaluation, and saving complete.') 